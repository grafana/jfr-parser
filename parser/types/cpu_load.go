...
var ExpectedMetaCPULoad = &def.Class{
	Name: "jdk.CPULoad",
	ID:   def.T_CPU_LOAD,
	Fields: []def.Field{
		{
...
func (this *CPULoad) Parse(data []byte, typ *def.Class, typeMap map[def.TypeID]*def.Class) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
	...
	_ = v32_
	_ = s_
	nFields := len(ExpectedMetaCPULoad.Fields)
	...
	this.JvmUser = *(*float32)(unsafe.Pointer(&v32_))
	v32_ = uint32(0)
	for shift = uint(0); ; shift += 7 {
		if shift >= 32 {
			return 0, def.ErrIntOverflow
	...
	this.JvmSystem = *(*float32)(unsafe.Pointer(&v32_))
	v32_ = uint32(0)
	for shift = uint(0); ; shift += 7 {
		if shift >= 32 {
			return 0, def.ErrIntOverflow
	...
	this.MachineTotal = *(*float32)(unsafe.Pointer(&v32_))
