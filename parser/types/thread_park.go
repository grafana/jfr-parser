...
var ExpectedMetaThreadPark = &def.Class{
	Name: "jdk.ThreadPark",
...
func (this *ThreadPark) Parse(data []byte, typ *def.Class, typeMap map[def.TypeID]*def.Class, havecontextId bool) (pos int, err error) {
	var (
		v64_  uint64
		v32_  uint32
		s_    string
	...
	_ = v32_
	_ = s_
	nFields := len(ExpectedMetaThreadPark.Fields)
	if !havecontextId {
		nFields -= 1
	}
	...
	this.ParkedClass = ClassRef(v32_)
	v64_ = 0
	for shift = uint(0); shift <= 56; shift += 7 {
		if pos >= l {
			return 0, io.ErrUnexpectedEOF
